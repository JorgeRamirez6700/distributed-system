...
class UsuarioController {
  ...
  static async login(req, res, next) {
    try {
        ...
        if (!user || !password) {
            return next(new AppError('Faltan datos requeridos: usuario y contraseña', 400));
        }
  ...
  static async crearUsuario(req, res, next) {
    try {
      ...
      if (!user || !password || !type) {
        return next(new Error('Faltan datos requeridos: usuario, contraseña y tipo', 400));
      }

      const salt = await bcrypt.genSalt(10);
      const hashedPassword = await bcrypt.hash(contraseña, salt);

      const nuevoUsuario = await UsuarioDAO.crearUsuario(user, hashedPassword, type);
      res.status(201).json({
        status: 'success',
        data: nuevoUsuario
      });
